      ...
      switch (temp[0]) {
            ...
            case "useContractConfig": {
                ...
                switch(contractconfig){
                    case "true":
                        useContractConfig = true;
                        break;
                    case "false":
                    default:
                        useContractConfig = false;
                        break;     
                }
                break;
            }    
            case "createPrivatePublicSet": {
                let mnemonic = temp[1];
                await createPrivatePublicSet(mnemonic);
                break;
            }    
            ...
            case "testGreeterPrivateDeployment": {
                ...
                if(inputValues.length == 8) {
                    ...
                    console.log("testGreeterPrivateDeployment: review the parameters list. It seems to be incorrect.")
                }
                break;
            }    
            case "testSetPrivateGreeter": 
            {
                let inputValues = temp[1].split(",");
                if(inputValues.length == 9) {
                    await setPrivateGreeterValues(inputValues[0], inputValues[1], inputValues[2], inputValues[3], inputValues[4], inputValues[5], inputValues[6], inputValues[7], inputValues[8]);
                } else {
                    console.log("testSetPrivateGreeter: review the parameters list. It seems to be incorrect.")
                }
                break;
            }    
            case "testGetPrivateGreeter": {
                let inputValues = temp[1].split(",");
                if(inputValues.length == 8) {
                    await getPrivateGreeterValues(inputValues[0], inputValues[1], inputValues[2], inputValues[3], inputValues[4], inputValues[5], inputValues[6], inputValues[7]);
                } else {
                    console.log("testGetPrivateGreeter: review the parameters list. It seems to be incorrect.")
                }
                break;
            }
            default:
...
async function createPrivatePublicSet(mnemonic) {
    if(!mnemonic.length) {
...
async function testLedgeriumToken(){
    ...
    if(!global.accountAddressList) {
        console.log("accountAddressList is empty! readKeyConfig flag is not set!");
        return;
    }
...
async function testGreetingContract() {
    ...
    try {
        ...
        if(!useContractConfig){
            ...
            let encodedABI = await utils.getContractEncodeABI(value[0], value[1], web3, constructorParameters);
            let transactionHash = await utils.sendMethodTransaction(ethAccountToUse, undefined, encodedABI, privateKey[ethAccountToUse], web3, 0);
            greeterPrivateAddress = transactionHash.contractAddress;
            utils.writeContractsINConfig("Greeter", greeterPrivateAddress);
        }
        else{
            greeterPrivateAddress = utils.readContractFromConfigContracts("Greeter");
        }
        console.log("Greeter deployedAddress ", greeterPrivateAddress);
        
        var greeting = new web3.eth.Contract(JSON.parse(value[0]), greeterPrivateAddress);
        ...
        var transactionObject = await utils.sendMethodTransaction(ethAccountToUse, greeterPrivateAddress, encodedABI, privateKey[ethAccountToUse], web3, 0);
...
async function deployGreeterPrivate(host1, host2, host3, host4, toPrivatePort, toPort1, otherPort1, otherPort2) {
    ...
    try {
        ...
        const txnParams = {
            ...
            isPrivate: true, 
            from: {
                privateKey: '0x' + global.privateKey[ethAccountToUse]
            }, 
        ...
        web31.eth.getTransactionCount(ethAccountToUse, 'pending', (err, nonce) => {
            ...
            newTx.then(function (tx){
                let greeterPrivateAddress = tx.contractAddress;
                console.log("Greeter deployed contract address: ", greeterPrivateAddress);
                console.log("Greeter deployed transactionHash: ", tx.transactionHash);
                utils.writeContractsINConfig("Greeter", greeterPrivateAddress);
                getPrivateGreeterValues(host1, host2, host3, host4, toPort1, otherPort1, otherPort2, greeterPrivateAddress);
            }).catch(function (err) {
                console.log(err);
            });
        });
        ...
        console.log("Error in deployGreeterPrivate(): " + error);
    }
}

async function getPrivateGreeterValues(host1, host2, host3, host4, toPort1, otherPort1, otherPort2, greeterPrivateAddress) {
    // Todo: Read ABI from dynamic source.
    var value = utils.readSolidityContractJSON("./build/contracts/Greeter");
    if((value.length <= 0) || (value[0] == "") || (value[1] == "")) {
        return;
    ...
    const contract1 = new web31.eth.Contract(JSON.parse(value[0]), greeterPrivateAddress);
    const contract2 = new web32.eth.Contract(JSON.parse(value[0]), greeterPrivateAddress);
    const contract3 = new web33.eth.Contract(JSON.parse(value[0]), greeterPrivateAddress);
    const contract4 = new web34.eth.Contract(JSON.parse(value[0]), greeterPrivateAddress);
...
async function setPrivateGreeterValues(host1, host2, host3, host4, toPrivatePort, toPort1, otherPort1, otherPort2, greeterPrivateAddress) {

    if(!useContractConfig && !greeterPrivateAddress) {
        greeterPrivateAddress = utils.readContractFromConfigContracts("Greeter");
        if(greeterPrivateAddress == "") {
            console.log("greeterPrivateAddress is undefined in contracts config")
            return;
        }
    }
    ...
    try {
        var ethAccountToUse = global.accountAddressList[0];
        var encodedABI = contract1.methods.setMyNumber(130000).encodeABI();
        let tlsOptions = {
            key: fs.readFileSync('./certs/cert.key'),
            clcert: fs.readFileSync('./certs/cert.pem'),
            allowInsecure: true
        ...
        var privateKeyFrom = '0x' + global.privateKey[ethAccountToUse];
        var gasPrice = await web3.eth.getGasPrice();
        const txnParams = {
            gasPrice: web3.utils.toHex(gasPrice),
            gasLimit: 4300000,
            to: greeterPrivateAddress,
            value: 0,
            data: encodedABI,        
            isPrivate: true,
            from: {
        ...
        console.log("Error in setPrivateGreeterValues(): " + error);
    }
}